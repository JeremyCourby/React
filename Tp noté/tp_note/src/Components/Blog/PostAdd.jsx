  ...
  return (<>
    <div onAnimationEnd={() => { setAnimationActive(false) }} className={ animationActive && "postDetails" } style={{ marginTop: "100px", position: 'fixed', marginLeft: '50%', width:'47%' }}>
      <div className="text-body post2 bg-body-secondary m-4 cadre_scroll ms-4 w-100">
        <h2 className="post-title mb-5"><input className="form-group form-control" placeholder="Title" rows="5" value={updateTitleInput} onChange={(event) => setUpdateTitleInput(event.target.value)}></input></h2>
        <textarea className="form-group form-control" rows="5" placeholder="Body" value={updateInput} onChange={(event) => setUpdateInput(event.target.value)}></textarea>
        <div style={{ float: 'right' }}>
          <button className="btn" type="button" onClick={() => props.addPost(updateInput, updateTitleInput)}><FontAwesomeIcon icon="fa-solid fa-plus" /></button>
        </div>
      </div>
    </div>
  </>);
